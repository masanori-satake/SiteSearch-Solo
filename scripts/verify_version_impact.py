...
def verify_version_impact():
    try:
        ...
        files = result.stdout.splitlines() or subprocess.run(["git", "diff", "--name-only", "main"], capture_output=True, text=True).stdout.splitlines()
...
if __name__ == "__main__":
    if not verify_version_impact(): sys.exit(1)
